...
class NodeNetwork(nn.Module):
    ...
    def forward(self, x, e, edge_index, x_mask):
        start, end = edge_index
...
class NodeMaskNetwork(nn.Module):
    """
    A module which computes new node features on the graph.
    For each node, it aggregates the neighbor node features
    (separately on the input and output side), and combines
    ...
    def __init__(self, input_dim, output_dim, hidden_activation='Tanh',
                 layer_norm=True):
        super(NodeMaskNetwork, self).__init__()
        self.network = make_mlp(input_dim*3, [output_dim]*3 + [1],
                                hidden_activation=hidden_activation,
                                output_activation=hidden_activation,
                                layer_norm=layer_norm)
...
class GNNSegmentClassifier(nn.Module):
    ...
    def __init__(self, input_dim=3, hidden_dim=8, n_graph_iters=3,
        ...
        # Setup the node mask network
        self.node_mask = NodeMaskNetwork(hidden_dim, hidden_dim,
                                        hidden_activation, layer_norm=layer_norm)
    ...
    def forward(self, inputs):
        ...
        x_mask = torch.ones(inputs.x.shape[0], 1)
        e = torch.ones(inputs.edge_index.shape[1])
        x_mask = self.node_mask(x, e, inputs.edge_index, x_mask)
        ...
        for i in range(self.n_graph_iters):
            ...
            # Apply edge network
            e = torch.sigmoid(self.edge_network(x, inputs.edge_index, x_mask))

            # Apply node network
            x = self.node_network(x, e, inputs.edge_index, x_mask)

            # apply the node mask network
            x_mask = self.node_mask(x, e, inputs.edge_index, x_mask)
        ...
        return self.edge_network(x, inputs.edge_index, x_mask)
