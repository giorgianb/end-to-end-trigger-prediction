...
def get_datasets(n_train, n_valid, n_test, 
        ...
        tmp_dir=None,
        n_epochs=16):
    gt_data = TrackDataset(gt_trigger_input_dir=gt_trigger_input_dir,
    ...
    pred_data = TrackDataset(gt_trigger_input_dir=None,
                        gt_nontrigger_input_dir=None,
                        pred_trigger_input_dir=pred_trigger_input_dir,
                        pred_nontrigger_input_dir=pred_nontrigger_input_dir,
                        n_trigger_samples=n_train+n_valid+n_test,
    ...
    total = (pred_trigger_input_dir is not None) + (pred_nontrigger_input_dir is not None)
    train_data, valid_data, test_data = random_split(pred_data, [total*n_train, total*n_valid, total*n_test])
    # Concat datasets
    train_data = torch.utils.data.ConcatDataset([train_data, gt_data])
