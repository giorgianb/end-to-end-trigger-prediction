...
class TrkDataset():
    ...
    def __init__(self, input_dir=None, filelist=None,
                ...
                load_complete_graph=False, add_geo_features=False, use_radius=False, 
    ...
    def __getitem__(self, index):
        ...
        m = self.momentums[index].reshape(-1, 3)
        # print(m.shape)
        return torch.from_numpy(track_vector).to(torch.float), n_tracks, torch.from_numpy(trigger), torch.from_numpy(m).to(torch.float)
...
def get_data_loaders(name, batch_size, **data_args):
    if name == 'trkvec-ecml-nomasked-physics' or name == 'trkvec-ecml-masked-physics':
