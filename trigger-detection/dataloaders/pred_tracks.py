...
def load_graph(filename, min_edge_probability, intt_required=False, tmp_dir=None):
    if tmp_dir is not None:
        cached_file = os.path.join(tmp_dir, os.path.basename(filename))
        if os.path.exists(cached_file):
            with open(cached_file, 'rb') as f:
                return pickle.load(f)
        else:
            return load_graph_direct(filename, min_edge_probability, intt_required=intt_required, tmp_dir=tmp_dir)
    else:
        return load_graph_direct(filename, min_edge_probability, intt_required=intt_required, tmp_dir=tmp_dir)


def load_graph_direct(filename, min_edge_probability, intt_required=False, tmp_dir=None):
...
class TrackDataset(object):
    ...
    def __getitem__(self, file_index):
        event_info = load_graph(self.filenames[file_index], self.min_edge_probability, self.intt_required, tmp_dir=self.tmp_dir)
