...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/blri_pretrained.yaml')
...
def train(data, model, lri_params, optimizer, epoch, output_dir):
    train_info = do_epoch(data, model, lri_params, epoch, optimizer)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model, lri_params, epoch):
    with torch.no_grad():
        val_info = do_epoch(data, model, lri_params, epoch, optimizer=None)
    return val_info

def do_epoch(data, model, lri_params, epoch, optimizer=None):
    ...
    total_size = 0
    total_selected = 0
    alpha, beta = lri_params['alpha'], lri_params['beta']
    trigger_keep_prob, nontrigger_keep_prob = lri_params['trigger_keep_prob'], lri_params['nontrigger_keep_prob']
    ...
    for batch in tqdm.tqdm(data):
        ...
        pred_labels, pred_mask_logits = model(tracks, mask)
        loss = 0
        ce_loss = F.cross_entropy(pred_labels, trig)
        loss += ce_loss
        accum_info['loss_ce'] += ce_loss.item() * batch_size
        if alpha < 0:
            # 1e-2 is an acceptable epsilon
            dist = torch.zeros(pred_mask_logits.shape).to(DEVICE, torch.float)
            m1 = is_trigger_track.unsqueeze(-1).repeat(1, 1, 2)
            dist[m1] = 1 - trigger_keep_prob
            m1[..., 0] = 0
            dist[m1] = trigger_keep_prob
            m2 = (~is_trigger_track).unsqueeze(-1).repeat(1, 1, 2)
            dist[m2] =  nontrigger_keep_prob
            m2[..., 1] = 0
            dist[m2] = 1 - nontrigger_keep_prob
        else:
            p1 = (1-alpha) * torch.ones(pred_mask_logits.shape[:-1]).to(DEVICE, torch.float)
            p2 = alpha * torch.ones(pred_mask_logits.shape[:-1]).to(DEVICE, torch.float)
            dist = torch.stack([p1, p2], dim=-1)

        # This way empty tracks do not contribute to the loss
        pred_mask_prob = torch.softmax(pred_mask_logits, dim=-1)

        mask = mask.to(torch.bool)
        dist[~mask] = pred_mask_prob[~mask]
        dist = dist.log()
        kl_loss = F.kl_div(dist, pred_mask_prob, reduction='batchmean')
        loss += beta*kl_loss
...
def execute_training(args, config):
    ...
    with open(config['classifier_config_path'], 'rb') as f:
    ...
    cmodel = CModel(**classifier_config['model'])
    cmodel = load_checkpoint(config['classifier_weights_path'], cmodel)
    model.classifier = cmodel
    ...
    params = [
            {'params': model.interpreter.parameters()},
            {'params': model.masker.parameters()},
    ...
    for epoch in range(1, config['epochs'] + 1):
        train_info = train(train_data, model, config['lri'], optimizer, epoch, config['output_dir'])
        table = make_table(
        ...
        val_info = evaluate(val_data, model, config['lri'], epoch)
        table = make_table(
    ...
    test_info = evaluate(test_data, best_model, config['lri'], config['epochs'] + 1)
    table = make_table(
    ...
    if args.use_wandb:
        ...
        wandb.log({"Test Loss" : test_info['loss_kl']})
