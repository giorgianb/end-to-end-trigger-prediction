...
def do_epoch(data, model, lri_params, epoch, finetune_epoch=False, optimizer=None):
    ...
    for batch in tqdm.tqdm(data):
        ...
        pred_labels, pred_sigma, good_hits = model(tracks, mask, finetune_epoch=finetune_epoch)
        loss = 0
...
def execute_training(args, config):
    ...
    params = [
            {'params': chain(model.interpreter.parameters(), model.noiser.parameters()), 'lr': oconfig['learning_rate']*1e-3},
