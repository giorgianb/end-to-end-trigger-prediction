...
def train(data, model_pnl, model, optimizer, epoch, output_dir, use_wandb=False, ce_weight=1):
    train_info = do_epoch(data, model_pnl, model, epoch, optimizer, ce_weight=ce_weight)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model_pnl, model, epoch, loss_config=None):
    with torch.no_grad():
        val_info = do_epoch(data, model_pnl, model, epoch, optimizer=None)
    return val_info


def do_epoch(data, model_pnl, model, epoch, optimizer=None, ce_weight=1):
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    name = config['name_on_wandb'] + f"-nhead{config['model']['num_heads']}-hid_d{config['model']['dim_hidden']}-use_radius{config['data']['use_radius']}-ln{config['model']['ln']}-ntrain{config['data']['n_train1']}*2-lr{config['optimizer']['learning_rate']}-{config['optimizer']['type']}-b{config['data']['batch_size']}"
    if args.use_wandb:
        wandb.init(
    ...
    config['model']['dim_input'] += 13*config['data']['add_geo_features'] + config['data']['use_radius']
